	...
	function get_troopTypesEnabled_from_storage(){
		var enabled_troops = {};
		if(localStorage.getItem("massScavenging") == null){
			for(var x = 0; x < game_data.units.length; x++){
				...
				enabled_troops[game_data.units[x]] = true;
			}
			localStorage.setItem("massScavenging", JSON.stringify(enabled_troops));
		}else{
			enabled_troops = JSON.parse(localStorage.getItem("massScavenging"));
		}
                                ...
                                <td colspan="10" id="massScavengeSophieTitle" style="text-align:center; width:auto; background-color:${headerColor}">
                                    <h2>
                                        <center style="margin:10px"><u>
                                                <font color="${titleColor}">GyĹąjtĂśgetĂŠs: csoportonkĂŠnt 50 falubĂłl</font>
                                            </u>
                                        </center>
                                    </h2>
                                </td>
    ...
    <div id="massScavengeSophie" class="ui-widget-content" style="position:fixed;background-color:${backgroundColor};cursor:move;z-index:50;">
        <table id="massScavengeSophieTable" class="vis" border="1" style="width: 100%;background-color:${backgroundColor};border-color:${borderColor}">
            <tr>
                <td colspan="10" id="massScavengeSophieTitle" style="text-align:center; width:auto; background-color:${headerColor}">
                    <h2>
                        <center style="margin:10px"><u>
                                <font color="${titleColor}">TĂśmeges gyĹąjtĂśgetĂŠs</font>
                            </u>
                        </center>
                    </h2>
                </td>
            ...
            <tr style="background-color:${backgroundColor}">
                <td style="text-align:center;background-color:${headerColor}" colspan="15">
                    <h2>
                        <center style="margin:10px"><u>
                                <font color="${titleColor}">VĂĄlassz egysĂŠgeket a gyĹąjtĂśgetĂŠshez</font>
                            </u></center>
                    </h2>
                </td>
            </tr>
            ...
            <tbody>
                <tr style="background-color:${backgroundColor}">
                    <td style="text-align:center;background-color:${headerColor}" colspan="4">
                        <h2>
                            <center style="margin:10px"><u>
                                    <font color="${titleColor}">VĂĄlassz gyĹąjtĂśgetĂŠsi kategĂłriĂĄkat</font>
                            </u></center>
                        </h2>
                    </td>
                </tr>
        ...
        <hr>
        <center>
            <font color="${titleColor}">HĂĄny ĂłrĂĄra legyenek kikĂźldve az egysĂŠgek? </font>
        </center>
        <br>
        <center><textarea id="runTime" cols="12" style="background-color:${backgroundColor};color:${titleColor};resize:none;" placeholder="Ărd ide"></textarea></center>
        <hr>
        <center><input type="button" class="btn evt-confirm-btn btn-confirm-yes" id="sendMass" onclick="readyToSend()" value="KikĂźldĹ gyorsgombok generĂĄlĂĄsa"></center>
        <hr>
        <center>
        <p>
    ...
    //create checkboxes, EV listeners and add them to the UI
    localUnitNames = [];
	...
	function change_storage_keys(unit, state){
		console.log(unit + " -> " + state);
		troopTypeEnabled[unit] = state;
		localStorage.setItem("massScavenging", JSON.stringify(troopTypeEnabled));
	}
